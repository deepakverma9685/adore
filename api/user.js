...
function checkAccessToken(data,connection,callback){
  ...
  if(data.access_token=='')
    { 
		var query = connection.query('SELECT * FROM devices WHERE device_id="'+data.device_id+'" AND device_type="'+data.device_type+'" ',function(err,rows,result)
		{  //console.log
									 ...
									 var query = connection.query(sql2,function(err,result){
									   ...
									   if(err){  
											  ...
											  else
											  {
																		...
																		else
																		{
																		   var sql5 = "SELECT id,name,prof_pic FROM user WHERE id='"+req.body.user_id+"'";
																			 var query = connection.query(sql5,function(err,result5){
																				  ...
																				  {  
																					  var result={status:1,message:'successfully matched '+req.body.swp_in,match:1,match_id:result4.insertId,match_user:result5[0]};
																					  return res.send(result);
																				  }
																	        });	
																		}
														       ...
														       else
														       {
																    var sql5 = "SELECT id,name,prof_pic FROM user WHERE id='"+req.body.user_id+"'";
																			 var query = connection.query(sql5,function(err,result5){
																				  ...
																				  {  
																					  var result={status:1,message:'successfully matched '+req.body.swp_in,match:1,match_id:result3[0].id,match_user:result5[0]};
																					  return res.send(result);
																				  }
																	        });
															   } 
												 ...
												 {
													 var result={status:1,message:'successfully swiped in '+req.body.swp_in,match:0};
													 return res.send(result);
												 }										   
											  }
								     });
						...
						else
						{
							   if(result.length > 0)
							   {
								   if(result[0].time_out ==0)
								   {
												...
												{  
													 if(result2.length>=1)
													 {
													     ...
													     var query = connection.query(sql3,function(err,result3)
													     {
															 ...
															 {
																	var result={status:1,message:'sucess',data:result2,dating_data:result3};
																	return res.send(result); 	
															 } 
														 });
								      ...
								      {
										  var result={status:0,message:'Your chat is not enable pleas answer question',data:'',qestion:0,match_id:req.body.match_id};
										  return res.send(result);
									  }
			   ...
			   if(result.status==1)
			   {
					...
					var query = connection.query(sql,function(err,result){ 
						...
						{  
							 ...
							 if(result.length >=1)
							 {   //res.send('if'+qes_array+''+ans_array); return false;
												...
												{  
													   ...
													   var query = connection.query(matching_update,function(err,result){
															   if(err){ 
																...
																{  
																	var result={status:1,message:'your answer submitted successfully & your chat is enabled'};
																	return res.send(result);
																} 	
														});
												} 	
														...
														{  
															   ...
															   var query = connection.query(matching_update,function(err,result){
																	   if(err){ 
																		...
																		{  
																			var result={status:1,message:'your answer submitted successfully & your chat is enabled on your hand'};
																			return res.send(result);
																		} 	
																});
														} 	
